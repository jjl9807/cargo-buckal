# @generated by `cargo buckal`

load("@buckal//:cargo_manifest.bzl", "cargo_manifest")
load("@buckal//:wrapper.bzl", "rust_binary", "rust_library", "rust_test")

filegroup(
    name = "vendor",
    srcs = glob(["**/**"]),
)

cargo_manifest(
    name = "manifest",
    vendor = ":vendor",
)

rust_binary(
    name = "libra",
    srcs = [":vendor"],
    crate = "libra",
    crate_root = "vendor/src/main.rs",
    edition = "2024",
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"] + select({
        "prelude//os/constraints:windows": select({
            "prelude//abi/constraints:gnu": [
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.53.1:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/winapi-x86_64-pc-windows-gnu/0.4.0:build-script-run[rustc_flags])",
            ],
            "DEFAULT": [
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.53.1:build-script-run[rustc_flags])",
            ],
        }),
        "DEFAULT": [],
    }),
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
        ":libra-lib",
    ],
)

rust_library(
    name = "libra-lib",
    srcs = [":vendor"],
    crate = "libra",
    crate_root = "vendor/src/lib.rs",
    edition = "2024",
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"],
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
    ],
)

rust_test(
    name = "unittest",
    srcs = [":vendor"],
    crate = "libra",
    crate_root = "vendor/src/lib.rs",
    edition = "2024",
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"] + select({
        "prelude//os/constraints:windows": select({
            "prelude//abi/constraints:gnu": [
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.53.1:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/winapi-x86_64-pc-windows-gnu/0.4.0:build-script-run[rustc_flags])",
            ],
            "DEFAULT": [
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.53.1:build-script-run[rustc_flags])",
            ],
        }),
        "DEFAULT": [],
    }),
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/assert_cmd/2.1.2:assert_cmd",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/serial_test/3.3.1:serial_test",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/tempfile/3.25.0:tempfile",
        "//third-party/rust/crates/testcontainers/0.25.2:testcontainers",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
    ],

    target_compatible_with = select({"//platforms:cross": ["config//:none"], "DEFAULT": []}),
)

rust_test(
    name = "ai_agent_test",
    srcs = [":vendor"],
    crate = "ai_agent_test",
    crate_root = "vendor/tests/ai_agent_test.rs",
    edition = "2024",
    env = {
        "CARGO_BIN_EXE_libra": "$(location :libra)",
    },
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"] + select({
        "prelude//os/constraints:windows": select({
            "prelude//abi/constraints:gnu": [
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.53.1:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/winapi-x86_64-pc-windows-gnu/0.4.0:build-script-run[rustc_flags])",
            ],
            "DEFAULT": [
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.53.1:build-script-run[rustc_flags])",
            ],
        }),
        "DEFAULT": [],
    }),
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/assert_cmd/2.1.2:assert_cmd",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/serial_test/3.3.1:serial_test",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/tempfile/3.25.0:tempfile",
        "//third-party/rust/crates/testcontainers/0.25.2:testcontainers",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
        ":libra-lib",
    ],

    target_compatible_with = select({"//platforms:cross": ["config//:none"], "DEFAULT": []}),
)

rust_test(
    name = "ai_chat_agent_test",
    srcs = [":vendor"],
    crate = "ai_chat_agent_test",
    crate_root = "vendor/tests/ai_chat_agent_test.rs",
    edition = "2024",
    env = {
        "CARGO_BIN_EXE_libra": "$(location :libra)",
    },
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"] + select({
        "prelude//os/constraints:windows": select({
            "prelude//abi/constraints:gnu": [
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.53.1:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/winapi-x86_64-pc-windows-gnu/0.4.0:build-script-run[rustc_flags])",
            ],
            "DEFAULT": [
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.53.1:build-script-run[rustc_flags])",
            ],
        }),
        "DEFAULT": [],
    }),
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/assert_cmd/2.1.2:assert_cmd",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/serial_test/3.3.1:serial_test",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/tempfile/3.25.0:tempfile",
        "//third-party/rust/crates/testcontainers/0.25.2:testcontainers",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
        ":libra-lib",
    ],
    target_compatible_with = select({"//platforms:cross": ["config//:none"], "DEFAULT": []}),
)

rust_test(
    name = "ai_dag_tool_loop_test",
    srcs = [":vendor"],
    crate = "ai_dag_tool_loop_test",
    crate_root = "vendor/tests/ai_dag_tool_loop_test.rs",
    edition = "2024",
    env = {
        "CARGO_BIN_EXE_libra": "$(location :libra)",
    },
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"] + select({
        "prelude//os/constraints:windows": select({
            "prelude//abi/constraints:gnu": [
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.53.1:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/winapi-x86_64-pc-windows-gnu/0.4.0:build-script-run[rustc_flags])",
            ],
            "DEFAULT": [
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.53.1:build-script-run[rustc_flags])",
            ],
        }),
        "DEFAULT": [],
    }),
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/assert_cmd/2.1.2:assert_cmd",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/serial_test/3.3.1:serial_test",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/tempfile/3.25.0:tempfile",
        "//third-party/rust/crates/testcontainers/0.25.2:testcontainers",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
        ":libra-lib",
    ],
    target_compatible_with = select({"//platforms:cross": ["config//:none"], "DEFAULT": []}),
)

rust_test(
    name = "ai_storage_flow_test",
    srcs = [":vendor"],
    crate = "ai_storage_flow_test",
    crate_root = "vendor/tests/ai_storage_flow_test.rs",
    edition = "2024",
    env = {
        "CARGO_BIN_EXE_libra": "$(location :libra)",
    },
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"] + select({
        "prelude//os/constraints:windows": select({
            "prelude//abi/constraints:gnu": [
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.53.1:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/winapi-x86_64-pc-windows-gnu/0.4.0:build-script-run[rustc_flags])",
            ],
            "DEFAULT": [
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.53.1:build-script-run[rustc_flags])",
            ],
        }),
        "DEFAULT": [],
    }),
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/assert_cmd/2.1.2:assert_cmd",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/serial_test/3.3.1:serial_test",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/tempfile/3.25.0:tempfile",
        "//third-party/rust/crates/testcontainers/0.25.2:testcontainers",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
        ":libra-lib",
    ],
    target_compatible_with = select({"//platforms:cross": ["config//:none"], "DEFAULT": []}),
)

rust_test(
    name = "cloud_storage_backup_test",
    srcs = [":vendor"],
    crate = "cloud_storage_backup_test",
    crate_root = "vendor/tests/cloud_storage_backup_test.rs",
    edition = "2024",
    env = {
        "CARGO_BIN_EXE_libra": "$(location :libra)",
    },
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"] + select({
        "prelude//os/constraints:windows": select({
            "prelude//abi/constraints:gnu": [
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.53.1:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/winapi-x86_64-pc-windows-gnu/0.4.0:build-script-run[rustc_flags])",
            ],
            "DEFAULT": [
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.53.1:build-script-run[rustc_flags])",
            ],
        }),
        "DEFAULT": [],
    }),
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/assert_cmd/2.1.2:assert_cmd",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/serial_test/3.3.1:serial_test",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/tempfile/3.25.0:tempfile",
        "//third-party/rust/crates/testcontainers/0.25.2:testcontainers",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
        ":libra-lib",
    ],
    target_compatible_with = select({"//platforms:cross": ["config//:none"], "DEFAULT": []}),
)

rust_test(
    name = "command_test",
    srcs = [":vendor"],
    crate = "command_test",
    crate_root = "vendor/tests/command_test.rs",
    edition = "2024",
    env = {
        "CARGO_BIN_EXE_libra": "$(location :libra)",
    },
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"] + select({
        "prelude//os/constraints:windows": select({
            "prelude//abi/constraints:gnu": [
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.53.1:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/winapi-x86_64-pc-windows-gnu/0.4.0:build-script-run[rustc_flags])",
            ],
            "DEFAULT": [
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.53.1:build-script-run[rustc_flags])",
            ],
        }),
        "DEFAULT": [],
    }),
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/assert_cmd/2.1.2:assert_cmd",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/serial_test/3.3.1:serial_test",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/tempfile/3.25.0:tempfile",
        "//third-party/rust/crates/testcontainers/0.25.2:testcontainers",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
        ":libra-lib",
    ],
    target_compatible_with = select({"//platforms:cross": ["config//:none"], "DEFAULT": []}),
)

rust_test(
    name = "e2e_mcp_flow",
    srcs = [":vendor"],
    crate = "e2e_mcp_flow",
    crate_root = "vendor/tests/e2e_mcp_flow.rs",
    edition = "2024",
    env = {
        "CARGO_BIN_EXE_libra": "$(location :libra)",
    },
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"] + select({
        "prelude//os/constraints:windows": select({
            "prelude//abi/constraints:gnu": [
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.53.1:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/winapi-x86_64-pc-windows-gnu/0.4.0:build-script-run[rustc_flags])",
            ],
            "DEFAULT": [
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.53.1:build-script-run[rustc_flags])",
            ],
        }),
        "DEFAULT": [],
    }),
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/assert_cmd/2.1.2:assert_cmd",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/serial_test/3.3.1:serial_test",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/tempfile/3.25.0:tempfile",
        "//third-party/rust/crates/testcontainers/0.25.2:testcontainers",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
        ":libra-lib",
    ],
    target_compatible_with = select({"//platforms:cross": ["config//:none"], "DEFAULT": []}),
)

rust_test(
    name = "intent_flow_test",
    srcs = [":vendor"],
    crate = "intent_flow_test",
    crate_root = "vendor/tests/intent_flow_test.rs",
    edition = "2024",
    env = {
        "CARGO_BIN_EXE_libra": "$(location :libra)",
    },
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"] + select({
        "prelude//os/constraints:windows": select({
            "prelude//abi/constraints:gnu": [
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.53.1:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/winapi-x86_64-pc-windows-gnu/0.4.0:build-script-run[rustc_flags])",
            ],
            "DEFAULT": [
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.53.1:build-script-run[rustc_flags])",
            ],
        }),
        "DEFAULT": [],
    }),
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/assert_cmd/2.1.2:assert_cmd",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/serial_test/3.3.1:serial_test",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/tempfile/3.25.0:tempfile",
        "//third-party/rust/crates/testcontainers/0.25.2:testcontainers",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
        ":libra-lib",
    ],
    target_compatible_with = select({"//platforms:cross": ["config//:none"], "DEFAULT": []}),
)

rust_test(
    name = "mcp_integration_test",
    srcs = [":vendor"],
    crate = "mcp_integration_test",
    crate_root = "vendor/tests/mcp_integration_test.rs",
    edition = "2024",
    env = {
        "CARGO_BIN_EXE_libra": "$(location :libra)",
    },
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"] + select({
        "prelude//os/constraints:windows": select({
            "prelude//abi/constraints:gnu": [
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.53.1:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/winapi-x86_64-pc-windows-gnu/0.4.0:build-script-run[rustc_flags])",
            ],
            "DEFAULT": [
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.53.1:build-script-run[rustc_flags])",
            ],
        }),
        "DEFAULT": [],
    }),
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/assert_cmd/2.1.2:assert_cmd",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/serial_test/3.3.1:serial_test",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/tempfile/3.25.0:tempfile",
        "//third-party/rust/crates/testcontainers/0.25.2:testcontainers",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
        ":libra-lib",
    ],
    target_compatible_with = select({"//platforms:cross": ["config//:none"], "DEFAULT": []}),
)

rust_test(
    name = "storage_r2_test",
    srcs = [":vendor"],
    crate = "storage_r2_test",
    crate_root = "vendor/tests/storage_r2_test.rs",
    edition = "2024",
    env = {
        "CARGO_BIN_EXE_libra": "$(location :libra)",
    },
    features = ["default"],
    rustc_flags = ["@$(location :manifest[env_flags])"] + select({
        "prelude//os/constraints:windows": select({
            "prelude//abi/constraints:gnu": [
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_gnu/0.53.1:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/winapi-x86_64-pc-windows-gnu/0.4.0:build-script-run[rustc_flags])",
            ],
            "DEFAULT": [
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.48.5:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.52.6:build-script-run[rustc_flags])",
                "@$(location //third-party/rust/crates/windows_x86_64_msvc/0.53.1:build-script-run[rustc_flags])",
            ],
        }),
        "DEFAULT": [],
    }),
    named_deps = {
        "rig": "//third-party/rust/crates/rig-core/0.30.0:rig-core",
    },
    os_deps = {
        "linux": ["//third-party/rust/crates/pager/0.16.1:pager"],
        "macos": ["//third-party/rust/crates/pager/0.16.1:pager"],
    },
    visibility = ["PUBLIC"],
    deps = [
        "//third-party/rust/crates/anyhow/1.0.102:anyhow",
        "//third-party/rust/crates/assert_cmd/2.1.2:assert_cmd",
        "//third-party/rust/crates/async-stream/0.3.6:async-stream",
        "//third-party/rust/crates/async-trait/0.1.89:async-trait",
        "//third-party/rust/crates/axum/0.8.8:axum",
        "//third-party/rust/crates/byte-unit/5.2.0:byte-unit",
        "//third-party/rust/crates/byteorder/1.5.0:byteorder",
        "//third-party/rust/crates/bytes/1.11.1:bytes",
        "//third-party/rust/crates/chrono/0.4.43:chrono",
        "//third-party/rust/crates/clap/4.5.60:clap",
        "//third-party/rust/crates/colored/3.1.1:colored",
        "//third-party/rust/crates/crc32fast/1.5.0:crc32fast",
        "//third-party/rust/crates/crossterm/0.28.1:crossterm",
        "//third-party/rust/crates/dagrs/0.6.0:dagrs",
        "//third-party/rust/crates/diffs/0.5.1:diffs",
        "//third-party/rust/crates/diffy/0.4.2:diffy",
        "//third-party/rust/crates/dirs/5.0.1:dirs",
        "//third-party/rust/crates/flate2/1.1.9:flate2",
        "//third-party/rust/crates/futures-core/0.3.32:futures-core",
        "//third-party/rust/crates/futures-util/0.3.32:futures-util",
        "//third-party/rust/crates/futures/0.3.32:futures",
        "//third-party/rust/crates/git-internal/0.7.0:git-internal",
        "//third-party/rust/crates/hex/0.4.3:hex",
        "//third-party/rust/crates/http/1.4.0:http",
        "//third-party/rust/crates/hyper-util/0.1.20:hyper-util",
        "//third-party/rust/crates/ignore/0.4.25:ignore",
        "//third-party/rust/crates/indicatif/0.18.4:indicatif",
        "//third-party/rust/crates/infer/0.19.0:infer",
        "//third-party/rust/crates/lazy_static/1.5.0:lazy_static",
        "//third-party/rust/crates/lru-mem/0.3.0:lru-mem",
        "//third-party/rust/crates/object_store/0.12.5:object_store",
        "//third-party/rust/crates/once_cell/1.21.3:once_cell",
        "//third-party/rust/crates/openssl/0.10.75:openssl",
        "//third-party/rust/crates/path-absolutize/3.1.1:path-absolutize",
        "//third-party/rust/crates/pathdiff/0.2.3:pathdiff",
        "//third-party/rust/crates/pulldown-cmark/0.12.2:pulldown-cmark",
        "//third-party/rust/crates/ratatui/0.29.0:ratatui",
        "//third-party/rust/crates/regex/1.12.3:regex",
        "//third-party/rust/crates/reqwest/0.12.28:reqwest",
        "//third-party/rust/crates/ring/0.17.14:ring",
        "//third-party/rust/crates/rmcp/0.13.0:rmcp",
        "//third-party/rust/crates/rpassword/7.4.0:rpassword",
        "//third-party/rust/crates/scopeguard/1.2.0:scopeguard",
        "//third-party/rust/crates/sea-orm/1.1.19:sea-orm",
        "//third-party/rust/crates/serde/1.0.228:serde",
        "//third-party/rust/crates/serde_json/1.0.149:serde_json",
        "//third-party/rust/crates/serial_test/3.3.1:serial_test",
        "//third-party/rust/crates/sha1/0.10.6:sha1",
        "//third-party/rust/crates/similar/2.7.0:similar",
        "//third-party/rust/crates/tempfile/3.25.0:tempfile",
        "//third-party/rust/crates/testcontainers/0.25.2:testcontainers",
        "//third-party/rust/crates/thiserror/2.0.18:thiserror",
        "//third-party/rust/crates/tokio-stream/0.1.18:tokio-stream",
        "//third-party/rust/crates/tokio-util/0.7.18:tokio-util",
        "//third-party/rust/crates/tokio/1.49.0:tokio",
        "//third-party/rust/crates/tower-http/0.6.8:tower-http",
        "//third-party/rust/crates/tower/0.5.3:tower",
        "//third-party/rust/crates/tracing-subscriber/0.3.22:tracing-subscriber",
        "//third-party/rust/crates/tracing/0.1.44:tracing",
        "//third-party/rust/crates/unicode-width/0.2.0:unicode-width",
        "//third-party/rust/crates/url/2.5.8:url",
        "//third-party/rust/crates/uuid/1.21.0:uuid",
        "//third-party/rust/crates/walkdir/2.5.0:walkdir",
        "//third-party/rust/crates/wax/0.6.0:wax",
        ":libra-lib",
    ],
    target_compatible_with = select({"//platforms:cross": ["config//:none"], "DEFAULT": []}),
)
